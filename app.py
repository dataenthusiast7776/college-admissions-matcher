...
def match_profiles(df, gpa, sat, act, eth, gen, ec_query):
    ...
    # initialize EC_matches as empty lists
    d['EC_matches'] = [[] for _ in range(len(d))]

    # keyword-based EC filtering
    if ec_query.strip():
...
def display_results(res):
    if res.empty:
        ...
        for _, r in res.iterrows():
            # Safely get EC matches (empty list if missing)
            ec_hits = r['EC_matches'] if 'EC_matches' in r.index else []
            ec_line = ""
            if ec_hits:
                ec_line = f"<br><b>ECs in common:</b> {', '.join(ec_hits)}"
            st.markdown(f"""
...
def main():
    ...
    <div style='font-size:1.05rem; color:white; margin-bottom:2.5rem; line-height:1.7; text-align:left;'>
      ...
      <br><br>
      Trained on a large dataset of 2958 (and growing!) real applicant profiles sourced from <i>r/collegeresults</i>, the matching algorithm will find you your college application twin, every single time.
      <br><br>
    ...
    with tabs[0]:
        ...
        res = match_profiles(df, user_gpa, user_sat, user_act, user_eth, user_gen, ec_query)
        display_results(res)
